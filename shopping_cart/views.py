...
from user_profiles.models import Profile, User
...
def order_details(request, **kwargs):
    ...
    return render(request, 'shopping_cart/shop/cart.html', context)
...
def make_payment(request, **kwargs):
    ...
    token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSMjU2In0.eyJjbGllbnRJZCI6ImRlMmFlYTg3LWZkZDEtNDMyYS04ZDIxLTdlNDg4ZmIxZWU0OSIsImV4cGlyZXMiOiIyMDIxLTAyLTI2VDEwOjM1OjQxLjg5NCIsInNlc3Npb25JZCI6IjNlNDNlMzI3LTE4ZTAtNGJlNy1hNmM2LWE1NmEzZDkxZjBiYSJ9.gVFmoP1S-xoxL4m5ll6OZ7lHyVgTHyzC6d59WQTqj08zPZLHK6VLUbivAWuYMuu7IYs_HubLJCOXjL9m074QkBeVxPI2xtn5NdMrSOVIv_Be2N5Rzbi4qWnQAQKPKWyR65bqi7VIFCt_yaRd9KlQ-CNjJpNjwujzgDZmw7jF8MGZsp436Kbhlsve9Dgyyqo3SULOc6QGny7xqLPUko8LmVZS2dIXl6UqjQLN7KjKYuc265mvh-in99XNrZZDibaoNC_QTtg5Ctfj6GjrG6PaymwBqsc-J46K_4fLGXNQA6HuZwXf8m-rbIgjMUdMT0-A5UFw0DyPPi9fZy6vECYj7A"
    ...
    return render(request, 'shopping_cart/shop/checkout.html', context)    
...
def update_transaction_records(request, order_id):
    ...
    # look at tutorial on how to send emails with sendgrid
    messages.info(request, "Thank you! Your purchase was successful!")
    return redirect(reverse('shopping_cart:my_orders'))#return a redirect to the users profile with their products they have purchased
...
def my_orders(request):
    ...
    print(orders)
        ...
        #     return order[0]
        # return 0
    messages.info(request, "Your order has been successfully recieved")    
    context ={
        'my_orders': orders
    }    
    return render(request, 'shopping_cart/shop/confirmation.html', context)


"""Get the Order by id or by the name of the owner"""
def search(request):
    form = SearchForm()
    """Get the selected option from the frontend and then process the search result based on the selected option"""
    if request.method == 'GET':
        query= request.GET.get('q')
        submitbutton= request.GET.get('submit')
        if query is not None:
            if request.GET.get('select') == 'reference':
                lookups= Q(ref_code=query)
                results= Order.objects.filter(lookups).distinct()
                print(results)
            elif request.GET.get('select') == 'name':    
                user = User.objects.get(username=query)
                profile = Profile.objects.get(user=user)
                results = Order.objects.filter(owner=profile)
                print(results)
            ...
            return render(request, 'shopping_cart/shop/search.html', context)
        else:
            return render(request, 'shopping_cart/shop/search.html')  
    else:
        return render(request, 'shopping_cart/shop/search.html')

        """Return to the view that displays the order in question and then redirect tothe view that refunds the amount to the user if necessary via mobile money"""

"""View an order that has already been paid for by a user """
