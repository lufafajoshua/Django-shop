...
class ChatConsumer(AsyncJsonWebsocketConsumer):
        ...
        try:
            ...
            if self.user in self.room.members.all():
                ...
                for user in self.room.members.all():
                    ...
                    #If the user is an agent, set the availability option to False
                    if user.user_type == 3:
                        user.useragent.availability=False
                        print(user.username)
                        print(user.useragent.availability)
                        user.save
            else:
    ...
    async def disconnect(self, close_code):
        ...
        for user in self.room.members.all():
            if user.user_type == 3:
                user.useragent.availability=True
                print(user.username)
                print(user.useragent.availability)
                user.save()
        ...
        #Check for the usera_type and set the availability option for the agent to True
